# PRD: Member Engagement Tab

## Overview
The Member Engagement tab provides detailed insights into member activity patterns, engagement metrics, and participation trends. This tab focuses on understanding how members interact with the program and benefit offerings.

## Objectives
- Track active member metrics (DAU, WAU, MAU)
- Understand engagement funnel and conversion rates
- Analyze member segmentation and behavior patterns
...
1. **MAU (Monthly Active Users)**
   - Definition: Unique members who engaged with the program in the past 30 days
   - Display: Formatted number with trend indicator
   - Trend: Percentage change from previous period

2. **DAU (Daily Active Users)**
   - Definition: Unique members who engaged with the program in the past 24 hours
   - Display: Formatted number with trend indicator
   - Trend: Percentage change from previous period

3. **MAC (Monthly Active with Orders)**
   - Definition: Members who completed at least one booking in the past 30 days
   - Display: Formatted number with trend indicator
   - Trend: Percentage change from previous period

4. **Repeat Users**
   - Definition: Members who have completed multiple transactions or bookings
   - Display: Formatted number with trend indicator
   - Trend: Percentage change from previous period

5. **Entitlement Utilization**
   - Definition: Percentage of allocated benefits used by members
...
- **Stages**:
  1. Eligible Members (100% baseline)
  2. Active Members (percentage of eligible)
  3. With Orders (percentage of eligible)
  4. Repeat Users (percentage of eligible)
- **Display**:
...
### Member Segmentation Table
- **Purpose**: Breakdown of members by engagement segment
- **Columns**:
  1. **Segment** - Member category (New, Active, Repeat)
  2. **Members** - Count of members in segment
  3. **Avg. Orders** - Average number of orders per member
...
### Calculation
1. **Eligible Members**: Total eligible (baseline = 100%)
2. **Active Members**: Count of active / Eligible × 100%
3. **With Orders**: Count with orders / Eligible × 100%
4. **Repeat Users**: Count repeat users / Eligible × 100%
...
|------------|------------|-------------|-------------|
| mau | Monthly Active Users - Unique members who engaged with the program in the past 30 days | Count of unique members with at least one engagement event in the past 30 days | |
| mauTrend | Percentage change in MAU compared to previous period | ((Current Period - Previous Period) / Previous Period) × 100% | |
| dau | Daily Active Users - Unique members who engaged with the program in the past 24 hours | Count of unique members with at least one engagement event in the past 24 hours | |
| dauTrend | Percentage change in DAU compared to previous period | ((Current Period - Previous Period) / Previous Period) × 100% | |
| mac | Monthly Active with Orders - Members who completed at least one booking in the past 30 days | Count of unique members with at least one completed order in the past 30 days | |
| macTrend | Percentage change in MAC compared to previous period | ((Current Period - Previous Period) / Previous Period) × 100% | |
| repeatUsers | Members who have completed multiple transactions or bookings | Count of unique members with 2 or more completed orders | |
...
| funnel.eligible | Total number of eligible members | Count of all customers eligible to participate in the program | |
| funnel.active | Count of active members | Count of unique members with at least one engagement in the period | |
| funnel.withOrders | Count of members who have placed at least one order | Count of unique members with at least one completed order | |
| funnel.repeat | Count of repeat users | Count of unique members with 2 or more completed orders | |
...
| dailyActiveTrend[].dau | Daily Active Users count for the date | Count of unique members with engagement on the specific date | |
| dailyActiveTrend[].wau | Weekly Active Users count for the date | Count of unique members with engagement in the 7 days ending on the date | |
| dailyActiveTrend[].mau | Monthly Active Users count for the date | Count of unique members with engagement in the 30 days ending on the date | |
| memberSegments | Array of member segmentation data objects | Collection of member segments with metrics | |
| memberSegments[].segment | Segment name (New, Active, Repeat) | Classification based on member engagement behavior | |
| memberSegments[].members | Count of members in the segment | Count of unique members classified in this segment | |
| memberSegments[].avgOrders | Average number of orders per member in segment | Total Orders in Segment / Members in Segment | |
| memberSegments[].value | Total value generated by segment | Sum of all transaction values for members in this segment | |
...
| engagementDepth | Average number of interactions per active member within the selected period | Total Interactions / Active Members | |
